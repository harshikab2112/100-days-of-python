# 🐍 100 Days of Code – Python Bootcamp

![Python](https://img.shields.io/badge/Python-3.x-blue?logo=python)
![Days](https://img.shields.io/badge/Days-0%2F100-lightgrey)
![Status](https://img.shields.io/badge/Challenge-In%20Progress-brightgreen)
![License](https://img.shields.io/badge/License-MIT-green)

This repository documents my journey through the **100 Days of Code: The Complete Python Pro Bootcamp by Angela Yu**.

The challenge is to **code in Python for 100 days** and build real-world projects while strengthening programming and problem-solving skills.

---

# 🚀 Course Overview

During this bootcamp, I will learn and practice:

- Python Fundamentals
- Data Types and Variables
- Control Flow and Loops
- Functions
- Object-Oriented Programming (OOP)
- File Handling
- APIs and Web Scraping
- Automation with Python
- GUI Development
- Web Development with Python
- Data Science Basics

Each day focuses on **hands-on coding and practical projects**.

---

# 📊 Progress Tracker

| Day | Topic / Project | Status |
|----|-----------------|--------|
| Day 1 | Working with Variables | ⏳ |
| Day 2 | Data Types | ⏳ |
| Day 3 | Control Flow | ⏳ |
| Day 4 | Randomisation & Lists | ⏳ |
| Day 5 | Loops | ⏳ |
| Day 6 | Functions | ⏳ |
| Day 7 | Hangman Project | ⏳ |
| Day 8 | Function Parameters | ⏳ |
| Day 9 | Dictionaries | ⏳ |
| Day 10 | Calculator Project | ⏳ |
| ... | ... | ... |
| Day 100 | Final Python Project | ⏳ |

✅ Completed  
⏳ In Progress

---

# 📈 Challenge Progress


Progress: 0%

[□□□□□□□□□□] 0/100 Days Completed


---

# 🗓 Daily Learning Log

### Day 1
- Learned Python variables
- Built **Band Name Generator**

### Day 2
- Learned Python data types
- Practiced type conversion

### Day 3
- Learned conditional statements
- Built **Treasure Island Game**

*(This log will be updated daily as I progress.)*

---

# 📂 Repository Structure


100-days-of-python/
│
├── Day01/
│ ├── main.py
│
├── Day02/
│ ├── main.py
│
├── Day03/
│ ├── main.py
│
├── Projects/
│
└── README.md


Each folder contains:

- Python exercises
- Mini projects
- Practice code

---

# 🛠 Tech Stack

- Python
- VS Code
- Git
- GitHub

---

# 🎯 Goals of This Challenge

- Build strong Python fundamentals
- Improve problem-solving skills
- Develop real-world Python projects
- Maintain daily coding discipline
- Document my programming journey

---

# 📚 Course Credit

This repository follows the course:

**100 Days of Code: The Complete Python Pro Bootcamp by Angela Yu**

---

⭐ Feel free to explore the repository and follow my progress!
